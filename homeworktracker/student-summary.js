...
function render() {
  ...
  const grouped = pendingItems.reduce((acc, item) => {
    const key = getStudentLabel(item);
    if (!acc[key]) {
      acc[key] = [];
    }
    acc[key].push(item);
        ...
        <ul>
          ${items
            .map(
              (item) => `
                <li>
                  <strong>${escapeHtml(item.homeworkTitle || "未填功課內容")}</strong>
                  <div class="meta">${escapeHtml(item.subject || "未填科目")} · ${formatDate(item.dueDate)}</div>
                </li>
              `
            )
            .join("")}
